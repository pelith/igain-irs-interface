...
export default function useTokenApprove(
...
): { approvalState: ApprovalInteractionState; approve: () => Promise<void> } {
  const { web3Provider, account, chainId } = web3Controller;
  const currentAllowance = useTokenAllowance(tokenAddress, account, spender, web3Controller);
  const [currentTransaction, setCurrentTransaction] = useState(null);
  const [transactionSucceeded, setTransactionSucceeded] = useState(false);
  const prevAddress = useRef('');

  const approvalState: ApprovalInteractionState = useMemo(() => {
  ...
  const approve = useCallback(async (): Promise<void> => {
    if (approvalState !== ExtraApprovalInteractionState.NOT_APPROVED) {
    ...
    try {
      tokenContract = new Contract(tokenAddress, ERC20_ABI, web3Provider.getSigner());
    } catch (error) {
  ...
  }, [approvalState, chainId, tokenAddress, spender, web3Provider]);
